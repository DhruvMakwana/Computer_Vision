...
class GoogLeNet:
	...
	def inception_module(x, num1x1, num3x3Reduce, num3x3, num5x5Reduce, num5x5, 
		...
		# define the third branch of the Inception module which are both 1x1 and 5x5 convolutions
		...
		fourth = MaxPooling2D((3, 3), strides = (1, 1), padding = "same", 
			name = stage + "_pool")(x)
		fourth = GoogLeNet.conv_module(fourth, num1x1Proj, 1, 1, (1, 1), chanDim, 
	...
	def build(width, height, depth, classes, reg = 0.0005):
		...
		x = GoogLeNet.conv_module(inputs, 64, 7, 7, (2, 2), chanDim, reg = reg, name = "block1")
